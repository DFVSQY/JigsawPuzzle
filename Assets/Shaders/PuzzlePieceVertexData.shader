/*
 * 和 PuzzlePiece 着色器同样的功能，主要更改是将连接状态（_ConnectedState）和网格信息（_NumCols、_NumRows、_CellIndex）
 * 修改为从顶点数据中读取，这样可以优化性能，降低DrawCall，需要搭配脚本组件 PuzzlePieceVertexData.cs 使用。
*/
Shader "Custom/PuzzlePieceVertexData"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct appdata_t
            {
                ...
                // 接收来自 C# 的自定义数据
                float4 uv1      : TEXCOORD1; // 对应 UIVertex.uv1 (Grid Info)
                float4 uv2      : TEXCOORD2; // 对应 UIVertex.uv2 (Connection)
            };
            ...
            struct v2f
            {
                ...
                // 传递给片元的数据
                float3 gridInfo : TEXCOORD3; // Cols, Rows, Index
            ...
            v2f vert(appdata_t v)
            {
                ...
                // 从顶点数据直接透传给 Fragment Shader
                OUT.gridInfo = v.uv1.xyz; // x=Cols, y=Rows, z=Index
                OUT.connState = v.uv2;    // x=T, y=R, z=B, w=L
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                float4 _ConnectedState = IN.connState; // T, R, B, L
